...
function Input({ type, ...props }: Props): JSX.Element {
    return (
        <input
            className="rounded-md border border-black/10 p-1 focus:outline-none"
            type={type || 'text'}
            {...props}
        />
    );
    ...
    //         </span>
    //         {p.type === 'textarea' ? (
    //             <textarea className="border border-black/10 resize max-w-[400px] min-w-52 min-h-20 max-h-40 w-96 outline-none rounded-md p-1"></textarea>
    //         ) : (
    //             <input
